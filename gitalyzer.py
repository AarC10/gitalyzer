...
def gather_data(repo: git.Repo, top_contrib_re_filter=None):
    """Gather data from the Git repository and filter contributors if needed."""
    pattern = re.compile(top_contrib_re_filter) if top_contrib_re_filter else None

    for commit in repo.iter_commits('--all'):
        year = commit.authored_datetime.year
        author = commit.author.name

        if pattern and pattern.search(author):
            continue
...
def graph_data(years: list[int], total_commits: list[int], total_contributors: list[int]):
    """Graph total contributions and contributors over time."""
...
def graph_contributors():
    """Graph contributions per year for the top 10 contributors."""
...
def graph_contributor_add_drop():
    """Graph contributor additions and drop-offs per year."""
...
def handle_figures(repo_path: str, contributors_graph: plt.Figure, contributions_graph: plt.Figure,
                   total_contributions_graph: plt.Figure, add_drop_graph: plt.Figure):
    """Save all figures to the output folder and display them."""
    ...
    add_drop_graph.savefig(os.path.join(output_folder, "contributor_add_drop.png"))
...
def main():
    ...
    years, total_commits, total_contributors = gather_data(repo, top_contrib_re_filter)

    total_contributions_graph, total_contributors_graph = graph_data(years, total_commits, total_contributors)
    top_contributors_graph = graph_contributors()
    add_drop_graph = graph_contributor_add_drop()

    handle_figures(repo_path, total_contributions_graph, total_contributors_graph, top_contributors_graph,
                   add_drop_graph)
